...
int main() {
  ...
  if (window1.isOpen()) {
    ...
    for (int i = 0; i != d; ++i) {
      std::cout << "\033c";
      std::cout << "S: " << count_S(board) << '\t' << "I: " << count_I(board) << "\t\t" << "R: " << count_R(board) << '\n';

      SIR::Point p2{i, count_S(board)};
      pointsS.push_back(p2);

      SIR::Point p{i, count_I(board)};
      pointsI.push_back(p);

      SIR::Point p1{i, count_R(board)};
      pointsR.push_back(p1);
